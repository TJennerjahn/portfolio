...
I copied this from a wiki article **verbatim** and it *just* worked. Just reload the config with `sudo nixos-rebuild switch`, reboot and be done with it. Even it hadn't worked, I could've just rolled back to the previous config (for which nixos conveniently creates grub entries) and be confident that no trace of the driver software would be left over. The worst case scenario on arch would require me to boot from a live-usb and chroot into my system and try to fix it. This *cannot* happen on NixOS.
